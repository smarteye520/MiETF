function portfolioNewSlideout(_args) {

	Ti.API.info('portfolioNewSlideout ');
	...
	portfolioNameInp.addEventListener('blur', function(e) {//findmefindme
		...
		if (!ignoreEvent) {
			...
			if (portfolioNameInp.value.length < 1)
				portfolioNameInp.value = 'Portfolio';
	...
	portfolioNameInp.addEventListener('change', function(e) {
		...
		if (e.source.value.length > 0) {
			animation.fadeIn(buttonHolderView, 500);
		} else {
    ...
    var rs = db.execute('SELECT a.*' + ' from vault a where vaultId = ?',  _args.vaultId);
    var vaultNum = 0;
    while (rs.isValidRow()) {
    ...
    rs.close();
    db.close();
    Ti.API.info('vaultNum '+vaultNum);
	var buttonMetal = Ti.UI.createButton({
		backgroundImage : 'images/latest/' + vaultColorDictionary[vaultNum-1].button,
		height : 128,
		width : 128,
		opacity : 1
	});
	...
	function saveSettings() {
		...
		*/
		...
		insertPortfolio(_args.vaultId, portfolioNameInp.value);
	...
	function setStatusMsg(msg) {
		...
		if (msg == 'Vault Saved!') {
			...
			if (true) {
				...
				statusMsg = 'Any portfolio options can be changed later by';
				statusMsgLine2 = 'clicking';
				statusMsgLine3 = '';
				
				statusMsgLbl.left = 115;
				statusMsgLbl.top = 150;
				statusMsgLblLine2.left = 594;
				statusMsgLblLine2.top = 150;
				statusMsgLblLine3.top = 150;
...
module.exports = portfolioNewSlideout;
