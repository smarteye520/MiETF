function portfolioEditSlideout(_args) {

    Ti.API.info('portfolioEditSlideout');
	...
	txtLbls.push(Ti.UI.createLabel({
		left : 125,
		top : 72,
		width : 300,
		height : Ti.UI.SIZE,
...
txtLbls[0].text = 'Portfolio Name';
	...
	var portfolioNameInp = Titanium.UI.createTextField({
		color : 'black',
		font : {
	...
	portfolioNameInp.addEventListener('blur', function(e) {  //findmefindme
		
	if (!ignoreEvent)
    { ignoreEvent = true;

		portfolioNameInp.font = {
			fontFamily : 'AvenirNextCondensed-Bold',
			fontSize : '20sp',
			fontWeight : 'Bold'
		};
		
		if (portfolioNameInp.value.length < 1) portfolioNameInp.value = 'Portfolio';
	...
	portfolioNameInp.addEventListener('change', function(e){
    	e.source.value = e.source.value.slice(0,16);
	});
	
	insideView.add(portfolioNameInp);
...
*/
	...
	function blinkPeriodForStep (step){
		...
		if (even == 0)	var text = txtMsgs[step] ;  
		if (even == 1)	var text = txtMsgs[step] + '_';
		...
		if (c == 3) {
			...
			//HANDLE the 3 STEPS
			if (step == 0) {
				//animation.popIn(portfolioNameInp);
				portfolioNameInp.focus();
			}
	...
	function typeTextForStatusMsg2LineStep2(isLast) {
		...
		if (even == 0)	var text = statusMsgLine3;
		if (even == 1)	var text = statusMsgLine3 + '_';
	...
	function blinkPeriodForStatusMsg(isLast) {
		...
		if (even == 0)	var text = statusMsg;
		if (even == 1)	var text = statusMsg + '_';
	...
	function saveSettings() {
		...
		//		rs = db.execute('INSERT INTO vault (vaultId, vaultName, vaultColor, vaultIconImg, vaultImg, isLocked, lockPasscode, vaultNum, vaultDisplayOrder, unlockTime, isSampleVault, vaultScreen) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 'Samples', '#3c89b6', 'images/latest/vault_1_sm.pn8', 'images/latest/vault_1_lg.pn8', 'NO', 'SET', '1', 0, 1406065700451, 'YES', 0);
			var vaultName = portfolioNameInp.value;
			...
			if (requirePasscodeSwitch.value) isSampleVault = 'NO';
			...
			*/
			...
			//Save the new Portfolio Name to Database
			updatePortfolio(_args.portfolioId, portfolioNameInp.value);

			// update portfolioSelect
			_args.portfolioSelect.resetPrepare();
	...
	function hideButtonHolderView(e){
		...
		statusMsgLbl.left = imgs.txtLblsLeft + 50; //38; // + 266;
	...
	function setStatusMsg(msg) {
		statusMsgLbl.text = '';
		statusMsgLbl.top = 565-11; //reset if in special location
		...
		if (msg == 'Vault Saved!') {
			...
			statusMsgLblLine2.font = {fontFamily : 'AmericanTypewriter',
					fontSize : '22sp',
					fontWeight : 'normal'
				};
			statusMsgLbl.color = 'black';
			statusMsgLbl.font = {fontFamily : 'AmericanTypewriter',
					fontSize : '22sp',
					fontWeight : 'normal'
				};
	...
	function resetOnClose() {
		...
		portfolioNameInp.value='';
		portfolioNameInp.opacity=0;
...
module.exports = portfolioEditSlideout; 